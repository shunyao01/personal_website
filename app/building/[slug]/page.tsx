...
import { getBuilding, getBuildingEntry, getSiteConfig, formatMonth } from "@/lib/content";
import { SiteFooter, SiteHeader } from "@/components/SiteShell";
import { Tag } from "@/components/Tag";

export async function generateStaticParams() {
  return getBuilding().map((entry) => ({ slug: entry.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const entry = getBuildingEntry(slug);
  if (!entry) return {};
    ...
    <div className="page-shell">
      ...
      <main id="top" tabIndex={-1} className="page-section section detail-page reveal">
        <div className="detail-content">
          <Link href="/building" className="detail-back">
            ← All building
          </Link>
